...
public class AppMainViewActivity extends Activity implements Runnable {
		...
		changeCategoryBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(isUserApp){
					...
					//设置标志位
					isUserApp = false;
					Toast.makeText(AppMainViewActivity.this, "用户自己的程序", 2000).show();
				}else{
					...
					//设置标志位
					isUserApp = true;
					Toast.makeText(AppMainViewActivity.this, "所有的程序", 2000).show();
				}
				gridView.setAdapter(new GridViewAdapter(showPackageInfos, AppMainViewActivity.this));
				listView.setAdapter(new ListViewAdapter(showPackageInfos, AppMainViewActivity.this));
			}
		});
		...
		changeViewBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(isListView == true){
					...
					//设置标志位
					isListView = false;
					Toast.makeText(AppMainViewActivity.this, "当前是列表视图", 2000).show();
					...
					//设置标志位
					isListView = true;
					Toast.makeText(AppMainViewActivity.this, "当前是网格视图", 2000).show();
	...
	OnItemClickListener listener = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			//创建一个Dialog用来进行选择
			AlertDialog.Builder builder = new AlertDialog.Builder(AppMainViewActivity.this);
			builder.setTitle("选项");
			//接收一个资源的ID
			builder.setItems(R.array.choice,new DialogInterface.OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					switch (which) {
					case 0:		    
						Intent serviceIntent = new Intent(AppMainViewActivity.this, FileTransferService.class);//注册客户端传文件的意图
						serviceIntent.setAction(FileTransferService.ACTION_SEND_FILE);
//						serviceIntent.putExtra(FileTransferService.EXTRAS_FILE_PATH, Environment.getExternalStorageDirectory()+"/"+packageInfo.packageName);
						serviceIntent.putExtra(FileTransferService.EXTRAS_FILE_PATH, "/data/app"+"/"+packageInfo.packageName);
						serviceIntent.putExtra(FileTransferService.EXTRAS_GROUP_OWNER_ADDRESS,
								adress);
						serviceIntent.putExtra(FileTransferService.EXTRAS_GROUP_OWNER_PORT, 8988);
						AppMainViewActivity.this.startService(serviceIntent);
						break;
					case 1:
						...
						if(activityInfo == null) {
							Toast.makeText(AppMainViewActivity.this, "没有任何activity", Toast.LENGTH_SHORT).show();
							return;
						}
